...
public class AnalysisVisitor extends MJElement.DefaultVisitor {
    ...
    LinkedHashMap<String, MJVarDecl> blockContext;

    // Info collection for translation phase
    /*
     * Map<MJFieldAccess, MJVarDecl> fieldAccess; Map<MJVarUse, MJVarDecl> varUse;
     * Map<MJMethodCall, MJMethodDecl> methodCall; Map<MJNewObject, MJClassDecl>
     * newObject;
     */
    Map<MJElement, MJElement> translationInfo;
    ...
    public AnalysisVisitor(List<TypeError> typeErrors) {
        ...
        this.translationInfo = new HashMap<>();
    ...
    public void visit(MJMethodDecl methodDecl) {
        ...
        for (MJVarDecl variable : formalParameters) {
            ...
            if (blockContext.containsKey(variable.getName())
                    && !currentClassCache.get("fields").contains(variable.getName()))
    ...
    public void visit(MJClassDecl classDecl) {
        ...
        for (MJVarDecl field : classFields) {
            if (blockContext.containsKey(field.getName()))
                ...
                // blockContext.put(field.getName(), field);
    ...
    private void typeCheck(MJStatement stmt) {
        stmt.match(new MJStatement.MatcherVoid() {
            ...
            public void case_VarDecl(MJVarDecl varDecl) {
                ...
                // typeCheck(varDecl.getType()).ifPresent(vtype ->
                // blockContext.put(varDecl.getName(), vtype));
                blockContext.put(varDecl.getName(), varDecl);
            ...
            public void case_StmtReturn(MJStmtReturn stmtReturn) {
                ...
                translationInfo.put(stmtReturn, currentMethodDecl);
            }
        });
    }
        ...
        return expr.match(new MJExprL.Matcher<Optional<MJType>>() {
            ...
            public Optional<MJType> case_VarUse(MJVarUse varUse) {
                ...
                if (blockContext.containsKey(varUse.getVarName())) {
                    // add info for translation phase
                    translationInfo.put(varUse, blockContext.get(varUse.getVarName()));
                    return Optional.of(blockContext.get(varUse.getVarName()).getType());
                } else {
                    translationInfo.put(varUse, fieldDecl.get());
            ...
            public Optional<MJType> case_FieldAccess(MJFieldAccess fieldAccess) {
                ...
                Optional<MJVarDecl> correspondingVD = getFieldDeclByName(((MJTypeClass) receiverType.get()).getName(),
                        fieldAccess.getFieldName());
                /*
                 * MJClassDecl cd = getClassDeclByName(((MJTypeClass)
                 * receiverType.get()).getName()); Optional<MJVarDecl> correspondingVD =
                 * cd.getFields().stream() .filter(vd ->
                 * vd.getName().equals(fieldAccess.getFieldName())).findAny();
                 */
                if (!correspondingVD.isPresent()) {
                    typeErrors.add(
                            new TypeError(fieldAccess, "Cannot find the requested field '" + fieldAccess.getFieldName()
                                    + "' in the class " + ((MJTypeClass) receiverType.get()).getName()));
                    return Optional.empty();
                }

                // adding info for translation phase
                translationInfo.put(fieldAccess, correspondingVD.get());
                // translationInfo.put(fieldAccess, receiverT)
    ...
    private Optional<MJType> typeCheck(MJExpr expr) {
        return expr.match(new MJExpr.Matcher<Optional<MJType>>() {
            ...
            public Optional<MJType> case_NewObject(MJNewObject newObject) {
                translationInfo.put(newObject, getClassDeclByName(newObject.getClassName()));
            ...
            public Optional<MJType> case_MethodCall(MJMethodCall methodCall) {
                ...
                // adding info for translation phase
                translationInfo.put(methodCall, md.get());
            ...
            public Optional<MJType> case_ExprBinary(MJExprBinary exprBinary) {
                ...
                return exprBinary.getOperator().match(new MJOperator.Matcher<Optional<MJType>>() {
                    ...
                    public Optional<MJType> case_Equals(MJEquals equals) {
                        if (!(leftType.get() instanceof NullType) && !(rightType.get() instanceof NullType)
                                && !leftType.get().structuralEquals(rightType.get())
                                && !(isSubtype(rightType.get(), leftType.get())
                                        || isSubtype(leftType.get(), rightType.get()))) {
     ...
     * 
     * @param className the class name from which to start looking for the field
